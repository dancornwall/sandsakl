# Auckland Central — sandsauckland.org.nz

Rebuild of [www.sandsauckland.org.nz](https://www.sandsauckland.org.nz), the site of **Sands Auckland Central** — a parent-run group supporting families across Auckland who have experienced the death of a baby, at any gestation and under any circumstance.

In the wider workspace this product is a **regional prototype and reference** for shared patterns. Status and open work: [ROADMAP.md](ROADMAP.md).

Workspace overview: [../../README.md](../../README.md)  
Shared assets: [../../shared/README.md](../../shared/README.md)
...
| [`v2-Claude/planning/`](v2-Claude/planning) | Content schedule, design system, and per-page layout briefs. | Planning complete for core pages. See its [README](v2-Claude/planning/README.md). |
| [`v3-build/`](v3-build) | Full draft static site (16 pages). | Draft for review — not launch-ready. See [v3-build/README.md](v3-build/README.md). |

## The build, in one paragraph

Static HTML/CSS/JS, **no build step, no framework** — a volunteer must be able to edit a page in plain HTML years from now. One shared template (header + footer + nav) on every page. Self-hosted open fonts (Fraunces + Public Sans). Real brand palette (deep violet `#472f92` + warm terracotta `#e5976a`), re-derived from the live theme CSS. Target **WCAG 2.2 AA**. Host on Netlify or Cloudflare Pages. All old WordPress URLs must 301-redirect (see [`_redirects`](v1-Claude/sands-auckland-prototype/_redirects)).
...
6. [v3-build/](v3-build) — draft site (`cd v3-build && python3 -m http.server 8123`).
